...
async def _durable_start_wait(main):
    ...
    names = [voting_runtime._row_name(main, row) for row in voting_runtime._players(main) if int(row["player_id"]) in blocked]
    blocked_text = "\n".join(f"• {html.escape(name)}" for name in names) if names else "• هیچ‌کس"
...
async def _start_target(main):
    ...
    voting_runtime._put(main, v)
    markup = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("🗳 رای می‌دهم", callback_data="vote:cast")) if v.get("mode") == voting_runtime.AUTO else None
    await main.bot.send_message(
        voting_runtime._gid(main),
...
async def _finish_round(main):
    ...
    if round_no == 1:
        ...
        for target in [int(x) for x in (v.get("targets") or [])]:
            if {int(x) for x in (votes.get(str(target)) or [])}:
    ...
    if round_no == 1:
        ...
        if defenders:
            ...
            for uid in defenders:
                names.append(f"• {html.escape(await _resolve_name(main, uid))}")
...
async def _round2(main, callback):
    ...
    for uid in candidates:
        ...
        kb.add(InlineKeyboardButton(f"{name} ✅", callback_data=f"vote:r2pick:{uid}"))
    ...
    await callback.message.edit_text("🔄 <b>بازیکنان دور دوم</b>\n\nفقط بازیکنانی که در دور اول به دفاع رفته‌اند در این فهرست هستند.", reply_markup=kb, parse_mode="HTML")
    await callback.answer()
...
def _remove_voting_handlers(main):
    ...
    for item in list(reg):
        ...
        if getattr(fn, "__module__", "") == "runtime.voting_runtime" and getattr(fn, "__name__", "") in names:
            continue
        kept.append(item)
    reg[:] = kept
...
def install(main):
    ...
    if dp is not None:
        ...
        async def end_handler(c):
            await only_mod(c); await _end(main, c)
