...
class AddonsMenuV2:
    ...
    async def allowed(self, uid):
        if uid == getattr(self.app, "moderator_id", None): return True
        for attr in ("admins", "group_admins"):
            for item in getattr(self.app, attr, None) or []:
                candidate = getattr(getattr(item, "user", None), "id", item)
                if candidate == uid: return True
        gid = self.group_id()
        if not gid: return False
        try:
            admins = await self.app.bot.get_chat_administrators(gid)
            admin_ids = {a.user.id for a in admins}; self.app.admins = admin_ids
            return uid in admin_ids
        except Exception: return False

    def settings(self):
        if not self.addons: return copy.deepcopy(DEFAULT_GROUP_SETTINGS)
        gid = self.group_id()
        if gid: self.addons.settings = self.addons.get_group_settings(gid)
        return self.addons.settings

    def save(self, settings):
        gid = self.group_id()
        if self.addons and gid:
            self.addons.set_group_settings(gid, settings); self.addons.settings = settings

    async def menu(self, callback):
        if callback.message.chat.type != "private": raise CancelHandler()
        if not await self.allowed(callback.from_user.id):
            await callback.answer("⛔ فقط مدیران گروه یا گرداننده دسترسی دارند.", show_alert=True); raise CancelHandler()
        s = self.settings()
    ...
    async def emoji(self, callback):
        ...
        text = (
            ...
            "با فعال بودن این گزینه، ربات در پیام‌های پشتیبانی‌شده از Custom Emoji استفاده می‌کند؛ "
            "با غیرفعال بودن، اموجی معمولی نمایش داده می‌شود.\n\n"
    ...
    async def visual(self, callback):
        ...
        kb = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton(f"🎨 نمایش نوبت اصلی: {'فعال' if s.get('primary', True) else 'غیرفعال'}", callback_data="adm2:add:toggle:primary"), InlineKeyboardButton(f"🟥 نمایش نوبت چالش: {'فعال' if s.get('challenge', True) else 'غیرفعال'}", callback_data="adm2:add:toggle:challenge"), InlineKeyboardButton("⬅️ امکانات اضافه", callback_data="adm2:add:menu"))
        ...
        if key in {"chatlock", "nightlock", "turnlock"}:
            if key in {"chatlock", "nightlock"}:
                try:
                    ...
                    await sync_group_permissions(self.app, self.group_id())
                except Exception:
                    pass
            await self.security(callback)
    ...
    async def reset(self, callback):
        if not await self.allowed(callback.from_user.id): await callback.answer("⛔ دسترسی ندارید.", show_alert=True); raise CancelHandler()
        self.save(copy.deepcopy(DEFAULT_GROUP_SETTINGS)); await self.menu(callback)

    def install(self):
        d = self.dp
        d.register_callback_query_handler(self.menu, lambda c: c.data in {"addons_menu", "adm2:add:menu"}, state="*")
