...
class GameStateMachine:
    ...
    It does not replace the existing runtimes; it composes them so Telegram
    handlers can depend on one state-machine boundary during the cut-over.
    """
    ...
    def snapshot(self, group_chat_id: int) -> dict[str, Any]:
        ...
        phase = Phase.LOBBY if raw_status in {"lobby", "waiting"} else Phase.RUNNING
        if turn:
            phase = Phase.CHALLENGE if pending else Phase.TURN
        elif raw_status in {"finished", "ended", "cancelled"}:
    ...
    def transition(self, group_chat_id: int, target: Phase) -> Transition:
        ...
        current = Phase(str(game.get("status") or "lobby").lower()) if str(game.get("status") or "lobby").lower() in {p.value for p in Phase} else Phase.LOBBY
        allowed = {
            ...
            Phase.CHALLENGE: {Phase.TURN, Phase.RUNNING, Phase.FINISHED},
            Phase.FINISHED: set(),
        }
    ...
    def recover(self, group_chat_id: int) -> dict[str, Any]:
        """Recover all persisted runtime information needed after process restart."""
