...
class ProfileEnhancements:
    ...
    async def set_gender(self, callback: types.CallbackQuery):
        ...
        if ok:
            self._invalidate(callback.from_user.id)
    ...
    async def save_nickname(self, message: types.Message, state: FSMContext):
        ...
        if result.rowcount > 0:
            ...
            if self.user_panel:
                await message.answer("✅ نام مستعار ذخیره شد.", reply_markup=self.user_panel._menu())
            else:
                await message.answer("✅ نام مستعار ذخیره شد.")
        else:
            await message.answer("❌ ذخیره نام مستعار انجام نشد.")
    ...
    async def transfer_target(self, message: types.Message, state: FSMContext):
        ...
        if not raw.isdigit() or int(raw) <= 0 or int(raw) == int(message.from_user.id):
            await message.answer("❌ آیدی مقصد باید یک عدد مثبت و متفاوت از حساب فعلی باشد.")
            return
        self.pending_transfers[int(message.from_user.id)] = int(raw)
        await state.set_state(ProfileStates.waiting_transfer_confirm)
        kb = InlineKeyboardMarkup(row_width=2).add(
            InlineKeyboardButton("✅ تأیید", callback_data="profile:transfer:confirm"),
            InlineKeyboardButton("❌ لغو", callback_data="profile:transfer:cancel"),
        )
    ...
    def transfer_account(self, source: int, target: int, actor: int, group_id: Optional[int]):
        ...
        try:
            with self._session() as s:
                ...
                for table, column in (("mafia_game_players", "player_id"), ("mafia_game_turns", "player_id"), ("mafia_challenges", "challenger_id"), ("mafia_challenges", "target_id"), ("mafia_ratings", "user_id")):
    ...
    async def transfer_confirm(self, callback: types.CallbackQuery, state: FSMContext):
        source = int(callback.from_user.id)
        target = int(self.pending_transfers.get(source) or 0)
        self.pending_transfers.pop(source, None)
        await state.finish()
        ok, msg = self.transfer_account(source, target, source, None)
        await callback.answer(msg, show_alert=True)
        if ok:
            await callback.message.edit_text("✅ انتقال حساب با موفقیت انجام شد.\n\nحساب مقصد اکنون هویت اصلی شما در Mafia Nights است.", parse_mode="HTML")

    async def transfer_cancel(self, callback: types.CallbackQuery, state: FSMContext):
        self.pending_transfers.pop(int(callback.from_user.id), None)
        await state.finish()
        await self.profile(callback)
    ...
    async def admin_transfer_command(self, message: types.Message):
        ...
        if len(parts) != 4 or not parts[2].isdigit() or not parts[3].isdigit():
            await message.reply("❌ قالب صحیح: <code>انتقال حساب آیدی_مبدأ آیدی_مقصد</code>", parse_mode="HTML")
            return
        actor, group = int(message.from_user.id), int(message.chat.id)
        if not self._is_manager(actor, group):
            await message.reply("❌ فقط مدیر گروه می‌تواند انتقال حساب انجام دهد.")
            return
        ok, msg = self.transfer_account(int(parts[2]), int(parts[3]), actor, group)
        await message.reply(msg)

    def register(self):
        dp = self.app.dp
        try:
            handlers = dp.callback_query_handlers.handlers
            if self.user_panel:
