"""Production Telegram command menu and slash-command adapters.

Only registers commands that have deterministic handlers in the current
production runtime. This is presentation/entry-point configuration and does
not modify game state.
"""
from __future__ import annotations

import logging
from aiogram import types
from aiogram.dispatcher.handler import CancelHandler


GROUP_COMMANDS = [
    types.BotCommand("start", "نمایش منوی اصلی"),
    types.BotCommand("ask", "پرسش از دستیار مافیا"),
    types.BotCommand("mafia", "پرسش از دستیار مافیا"),
    types.BotCommand("ai_on", "فعال‌سازی هوش مصنوعی"),
    types.BotCommand("ai_off", "غیرفعال‌سازی هوش مصنوعی"),
    types.BotCommand("ai_status", "وضعیت هوش مصنوعی"),
    types.BotCommand("ai_key", "ثبت امن API Key"),
    types.BotCommand("kb_list", "لیست پایگاه دانش"),
    types.BotCommand("kb_add", "افزودن مطلب دانش"),
    types.BotCommand("kb_publish", "انتشار مطلب دانش"),
    types.BotCommand("ask", "پرسش از دستیار مافیا"),
    types.BotCommand("mafia", "پرسش از دستیار مافیا"),
    types.BotCommand("commands", "فهرست دستورات متنی"),
    types.BotCommand("newgame", "بازی جدید"),
    types.BotCommand("join", "ورود"),
    types.BotCommand("leave", "خروج"),
    types.BotCommand("stats", "آمار"),
    types.BotCommand("sub", "افزودن بازیکن جایگزین"),
    types.BotCommand("sub_list", "لیست جایگزین‌ها"),
    types.BotCommand("sub_del", "حذف جایگزین"),
    types.BotCommand("vote", "رأی‌گیری"),
    types.BotCommand("end", "اتمام بازی"),
    types.BotCommand("night", "فاز شب"),
    types.BotCommand("day", "شروع روز"),
    types.BotCommand("chief", "تغییر سردست"),
    types.BotCommand("remove", "حذف بازیکن"),
    types.BotCommand("start_round", "شروع دور"),
    types.BotCommand("next", "نکست"),
    types.BotCommand("next_settings", "تنظیم نکست"),
    types.BotCommand("challenge_settings", "تنظیم چالش"),
    types.BotCommand("mute", "سکوت"),
    types.BotCommand("unmute", "حذف سکوت"),
    types.BotCommand("extra", "ترن اضافه"),
    types.BotCommand("nickname_set", "تنظیم مستعار"),
    types.BotCommand("nickname_get", "نام مستعار"),
    types.BotCommand("nickname_del", "حذف مستعار"),
    types.BotCommand("nickname_list", "لیست مستعار"),
    types.BotCommand("tagall", "تگ همه"),
    types.BotCommand("tagadmins", "تگ مدیران"),
    types.BotCommand("taglist", "تگ لیست"),
    types.BotCommand("chatlock", "قفل چت"),
    types.BotCommand("chatunlock", "باز کردن چت"),
    types.BotCommand("nightlock", "قفل شب"),
    types.BotCommand("nightunlock", "باز کردن شب"),
    types.BotCommand("turnlock", "قفل نوبت"),
    types.BotCommand("turnunlock", "باز کردن نوبت"),
]

PRIVATE_COMMANDS = [
    types.BotCommand("start", "نمایش منوی اصلی"),
    types.BotCommand("commands", "فهرست دستورات متنی"),
    types.BotCommand("stats", "نمایش آمار بازی"),
    types.BotCommand("nickname_get", "نام مستعار"),
    types.BotCommand("nickname_set", "تنظیم مستعار"),
]


def install(app):
    if getattr(app, "_telegram_commands_installed", False):
        return False

    dp = app.dp

    async def new_game(message: types.Message):
        if message.chat.type not in {"group", "supergroup"}:
            await message.reply("⚠️ این دستور فقط داخل گروه قابل استفاده است.")
            raise CancelHandler()
        handler = getattr(app, "_canonical_new_game_handler", None)
        if handler is None:
            await message.reply("⚠️ مسیر ایجاد بازی در دسترس نیست.")
            raise CancelHandler()
        from types import SimpleNamespace
        callback = SimpleNamespace(
            message=message,
            from_user=message.from_user,
            data="fl_new",
            answer=message.answer,
            _from_text_command=True,
        )
        await handler(callback)
        raise CancelHandler()

    dp.register_message_handler(
        new_game,
        commands=["newgame"],
        state="*",
        content_types=[types.ContentTypes.TEXT],
    )

    async def tagall(message: types.Message):
        from commands import cmd_tag_all
        await cmd_tag_all(message, app)
        raise CancelHandler()

    async def tagadmins(message: types.Message):
        from commands import cmd_tag_admins
        await cmd_tag_admins(message, app)
        raise CancelHandler()

    dp.register_message_handler(tagall, commands=["tagall"], state="*")
    dp.register_message_handler(tagadmins, commands=["tagadmins"], state="*")

    async def register_remote_commands():
        try:
            # A chat-specific scope guarantees that Telegram shows the commands
            # when the user types "/" in the configured Mafia group.
            await app.bot.set_my_commands(
                GROUP_COMMANDS,
                scope=types.BotCommandScopeChat(chat_id=int(app.ALLOWED_GROUP_ID)),
            )
            await app.bot.set_my_commands(PRIVATE_COMMANDS)
            logging.info(
                "TELEGRAM COMMAND MENU installed group=%s commands=%s",
                app.ALLOWED_GROUP_ID,
                [c.command for c in GROUP_COMMANDS],
            )
        except Exception:
            logging.exception("TELEGRAM COMMAND MENU installation failed")

    app._telegram_commands_installed = True
    app._register_telegram_commands = register_remote_commands
    logging.info("TELEGRAM COMMAND SURFACE installed")
    return True
