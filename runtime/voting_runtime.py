...
def _rt(main): return getattr(main, "runtime", None)
def _game(main):
    rt, gid = _rt(main), _gid(main)
    return rt.state.active_game(gid) if rt and gid else None

def _state(main): return dict((_game(main) or {}).get("state") or {})
...
def _players(main):
    ...
    if game and rt:
        try:
            return [dict(x) for x in rt.state.games.list_players(game["id"])
...
def _name(main, uid, seat=None):
    try:
        ...
        for method in ("get_nick", "get"):
            ...
            if fn:
                value = fn(int(uid))
                if value and str(value).strip() not in {"?", "❓", "None", "بازیکن"}: return str(value)
    except Exception: pass
    try:
...
def _v(main):
    payload = _state(main); voting = payload.get("voting")
    if not isinstance(voting, dict):
        voting = _default(main); payload["voting"] = voting; _save(main, payload)
    return voting
...
async def _settings(main, callback):
    v = _v(main)
    text = ("🗳 <b>تنظیمات رای‌گیری</b>\n\n" f"⏱ زمان انتظار: {v['wait_seconds']} ثانیه\n"
            f"⏱ زمان هر رای: {v['vote_seconds']} ثانیه\n" f"🚫 بدون حق رای: {len(v.get('vote_rights_taken', []))} نفر\n"
            f"🗳 نوع: {'خودکار' if v.get('mode') == AUTO else 'دستی'}")
    await callback.message.edit_text(text, reply_markup=_settings_kb(v), parse_mode="HTML"); await callback.answer()

async def _start_target(main):
    v = _v(main); targets = list(v.get("targets") or []); idx = int(v.get("target_index") or 0)
    if idx >= len(targets): return await _finish_round(main)
    target = int(targets[idx]); rows = {int(x["player_id"]): x for x in _players(main)}
    name = _name(main, target, rows.get(target, {}).get("seat")); now = time.time(); deadline = now + int(v["vote_seconds"])
    v["phase"], v["started_at"], v["deadline"] = "voting", now, deadline; v.setdefault("votes", {}).setdefault(str(target), []); _put(main, v)
...
async def _timer(main, deadline, expected):
    await asyncio.sleep(max(0, float(deadline) - time.time())); v = _v(main)
...
async def _end_target(main):
    v = _v(main); targets = list(v.get("targets") or []); idx = int(v.get("target_index") or 0)
    if idx >= len(targets): return await _finish_round(main)
    target = int(targets[idx]); voted = {int(x) for x in (v.get("votes") or {}).get(str(target), [])}; rows = {int(x["player_id"]): x for x in _players(main)}
    target_name = _name(main, target, rows.get(target, {}).get("seat")); voter_text = "\n".join(f"• {html.escape(_name(main, uid, rows.get(uid, {}).get('seat')))}" for uid in voted) or "• هیچ‌کس"
    ...
    v["target_index"], v["started_at"], v["deadline"] = idx + 1, None, None; _put(main, v); await _start_target(main)

async def _finish_round(main):
    v = _v(main); v["phase"], v["deadline"] = "round_finished", None; _put(main, v)
...
def install(main):
    ...
    async def only_mod(c):
        if int(c.from_user.id) != int(getattr(main, "moderator_id", -1) or -1): await c.answer("⛔ فقط گرداننده دسترسی دارد.", show_alert=True); raise CancelHandler()
    async def settings(c): await only_mod(c); await _settings(main,c)
    async def wait_menu(c): await only_mod(c); v=_v(main); await c.message.edit_text("⏱ <b>زمان انتظار رای‌گیری</b>",reply_markup=_choices("wait",WAIT_OPTIONS,v["wait_seconds"]),parse_mode="HTML"); await c.answer()
    async def duration_menu(c): await only_mod(c); v=_v(main); await c.message.edit_text("⏱ <b>زمان هر رای</b>",reply_markup=_choices("duration",VOTE_OPTIONS,v["vote_seconds"]),parse_mode="HTML"); await c.answer()
    async def choose_wait(c): await only_mod(c); v=_v(main); v["wait_seconds"]=int(c.data.rsplit(":",1)[1]); _put(main,v); await _settings(main,c)
    async def choose_duration(c): await only_mod(c); v=_v(main); v["vote_seconds"]=int(c.data.rsplit(":",1)[1]); _put(main,v); await _settings(main,c)
    async def rights(c): await only_mod(c); v=_v(main); await c.message.edit_text("🚫 <b>گرفتن حق رای</b>",reply_markup=_rights_kb(main,v),parse_mode="HTML"); await c.answer()
    async def toggle_right(c): await only_mod(c); v=_v(main); uid=int(c.data.rsplit(":",1)[1]); s={int(x) for x in v.get("vote_rights_taken",[])}; s.symmetric_difference_update({uid}); v["vote_rights_taken"]=sorted(s); _put(main,v); await rights(c)
    async def mode(c): await only_mod(c); v=_v(main); await c.message.edit_text("🗳 <b>نوع رای‌گیری</b>",reply_markup=_choices("mode",(AUTO,MANUAL),v.get("mode")),parse_mode="HTML"); await c.answer()
    async def choose_mode(c): await only_mod(c); v=_v(main); v["mode"]=c.data.rsplit(":",1)[1]; _put(main,v); await _settings(main,c)
    async def start(c):
        ...
        await c.answer("🗳 رای‌گیری آماده شد."); await _start_wait(main); raise CancelHandler()
    async def cast(c):
        v=_v(main)
        ...
        if idx>=len(targets): await c.answer("⚠️ این رای‌گیری تمام شده است.",show_alert=True); raise CancelHandler()
        target=int(targets[idx]); votes=v.setdefault("votes",{}); bucket={int(x) for x in votes.setdefault(str(target),[])}
    ...
    async def night(c):
        await only_mod(c); rt,gid=_rt(main),_gid(main)
        if rt and gid:
    ...
    if not getattr(stable,"_voting_end_day_wrapped",False):
        original=stable._end_day
        async def end_day_with_voting(engine_main):
            if getattr(engine_main,"_stable_day_ended",False): return
            engine_main._stable_day_ended=True; engine_main._stable_day_active=False; engine_main._stable_phase="ended"; engine_main._gm_extra_turn_active=False; engine_main._gm_extra_phase=False
            task=getattr(engine_main,"turn_timer_task",None)
            if task and not task.done(): task.cancel()
            engine_main.turn_timer_task=None; engine_main.challenge_mode=False; engine_main.pending_challenges={}; engine_main.active_challenger_seats=set(); engine_main.current_turn_index=len(getattr(engine_main,"turn_order",[]) or [])
            gid=_gid(engine_main); mid=getattr(engine_main,"current_turn_message_id",None)
            if gid and mid:
            ...
            await engine_main.bot.send_message(gid,"✅ همه بازیکنا صحبت کردن. فاز روز تموم شد.",reply_markup=_day_end_kb())
        stable._end_day=end_day_with_voting; stable._voting_end_day_wrapped=True
    main._voting_runtime_installed=True; return True
