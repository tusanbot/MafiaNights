...
class FeatureRepository(DatabaseRepository):
        ...
        with self.SessionLocal() as s:
            ...
            for r in rows:
                if r["result"] == "win":
                    ...
                    streak = 0
            return {"games": games,"wins": wins,"challenges": challenges,"clean_games": clean,"delta": delta,"avg_game_score": (sum(scores) / games) if games else 0,"best_win_streak": best_streak}
    ...
    def sync_achievements(self, uid: int) -> list[dict[str, Any]]:
        ...
        with self.SessionLocal() as s:
            for a in ACHIEVEMENTS:
                complete = stats.get(a["metric"], 0) >= a["target"] and (not a.get("requires_games") or stats["games"] >= a["requires_games"])
                ...
                if row: unlocked.append(a); continue
    ...
    def event(self, event_id: int) -> dict[str, Any] | None:
        with self.SessionLocal() as s:
            row = s.execute(text("select * from public.mafia_events where id=:id"), {"id": int(event_id)}).mappings().first(); return dict(row) if row else None
    ...
    def register_event_player(self, event_id: int, uid: int) -> bool:
        with self.SessionLocal() as s:
            if s.execute(text("select status from public.mafia_events where id=:id"), {"id": int(event_id)}).scalar_one_or_none() != "active": return False
            s.execute(text("insert into public.mafia_event_players(event_id,player_id) values(:eid,:uid) on conflict(event_id,player_id) do update set status='registered',updated_at=now()"), {"eid": int(event_id),"uid": int(uid)}); s.commit(); return True
    ...
    def update_event(self, event_id: int, **values) -> None:
        ...
        clauses=[]; params={"id":int(event_id)}
    ...
    def create_stage(self, event_id: int, name: str, stage_type: str) -> int:
        with self.SessionLocal() as s:
            ...
            row=s.execute(text("insert into public.mafia_event_stages(event_id,name,stage_type,stage_order) values(:eid,:name,:type,:ord) returning id"), {"eid":int(event_id),"name":name,"type":stage_type,"ord":order}).scalar_one(); s.commit(); return int(row)
    ...
    def assign_group(self, stage_id: int, uid: int, group_no: int) -> None:
        with self.SessionLocal() as s: s.execute(text("insert into public.mafia_event_stage_players(stage_id,player_id,group_no) values(:sid,:uid,:grp) on conflict(stage_id,player_id) do update set group_no=excluded.group_no,updated_at=now()"), {"sid":int(stage_id),"uid":int(uid),"grp":int(group_no)}); s.commit()

    def assign_score(self, stage_id: int, uid: int, score: int) -> None:
        with self.SessionLocal() as s: s.execute(text("insert into public.mafia_event_stage_players(stage_id,player_id,score) values(:sid,:uid,:score) on conflict(stage_id,player_id) do update set score=excluded.score,updated_at=now()"), {"sid":int(stage_id),"uid":int(uid),"score":int(score)}); s.commit()
    ...
    def auto_group(self, stage_id: int, count: int) -> None:
        with self.SessionLocal() as s:
            ...
            for i,uid in enumerate(rows): s.execute(text("insert into public.mafia_event_stage_players(stage_id,player_id,group_no) values(:sid,:uid,:grp) on conflict(stage_id,player_id) do update set group_no=excluded.group_no,updated_at=now()"), {"sid":int(stage_id),"uid":int(uid),"grp":(i%max(1,int(count)))+1})
            s.execute(text("update public.mafia_events set grouping_mode='auto',updated_at=now() where id=(select event_id from public.mafia_event_stages where id=:sid)"), {"sid":int(stage_id)}); s.commit()
    ...
    def replace_player(self, event_id: int, old_uid: int, new_uid: int) -> None:
        with self.SessionLocal() as s:
            ...
            s.execute(text("update public.mafia_event_stage_players set player_id=:new,status='active',updated_at=now() where player_id=:old and stage_id in (select id from public.mafia_event_stages where event_id=:eid)"), {"eid":int(event_id),"old":int(old_uid),"new":int(new_uid)}); s.commit()

    def incident(self, game_id: str) -> dict[str, Any] | None:
        with self.SessionLocal() as s:
            row=s.execute(text("select * from public.mafia_game_incidents where game_id=:gid"), {"gid":str(game_id)}).mappings().first(); return dict(row) if row else None

    def save_incident(self, game_id: str, content: list[str], actor: int, finalized: bool, edit=False) -> None:
        with self.SessionLocal() as s:
            payload=json.dumps(content,ensure_ascii=False); row=s.execute(text("select id,version from public.mafia_game_incidents where game_id=:gid"), {"gid":str(game_id)}).mappings().first()
            if row:
...
class MafiaProgressEvents:
    def __init__(self, app):
        self.app=app; self.repo=FeatureRepository(); self.ratings=RatingRepository(); app._achievement_engine=self.repo
    ...
    async def stats_override(self,callback):
        ...
        kb=InlineKeyboardMarkup(row_width=2); kb.row(InlineKeyboardButton("🏆 دستاوردها",callback_data="profile:advanced:achievements"),InlineKeyboardButton("⬅️ پروفایل",callback_data="ustats:profile")); await callback.message.edit_text(text_body,parse_mode="HTML",reply_markup=kb); await callback.answer()
    ...
    async def event_detail(self,callback):
        ...
        if stages:
            lines += ["📚 مراحل:"]
            for stage in stages:
                lines.append(f"• {stage['name']} ({stage['stage_type']}) — {stage['status']}")
    ...
    async def event_players(self,callback):
        eid=int(str(callback.data).split(":")[-1]); players=self.repo.event_players(eid); lines=["👥 <b>بازیکنان اونت</b>",""]
        for i,p in enumerate(players,1): lines.append(f"{i}. {html.escape(self._name(p))} — {'ثبت‌نام‌شده' if p.get('status')=='registered' else p.get('status')}")
        lines.append(f"\n⭐ بازیکنان ثبت‌نام‌شده: {sum(p.get('status')=='registered' for p in players)}")
        kb=InlineKeyboardMarkup(row_width=2)
        if await self._admin(callback):
            kb.row(InlineKeyboardButton("➕ افزودن بازیکن",callback_data=f"mfeature:event_add_player:{eid}"),InlineKeyboardButton("🗑 حذف بازیکن",callback_data=f"mfeature:event_remove_player:{eid}")); kb.add(InlineKeyboardButton("⬅️ مدیریت اونت",callback_data=f"mfeature:event_admin:{eid}"))
    ...
    async def event_admin(self,callback):
        ...
        eid=int(str(callback.data).split(":")[-1]); e=self.repo.event(eid); kb=InlineKeyboardMarkup(row_width=2)
        kb.row(InlineKeyboardButton("📝 تغییر نام",callback_data=f"mfeature:event_edit_name:{eid}"),InlineKeyboardButton("🗓 تغییر زمان",callback_data=f"mfeature:event_edit_time:{eid}")); kb.row(InlineKeyboardButton("👥 بازیکنان",callback_data=f"mfeature:event_players:{eid}"))
        kb.row(InlineKeyboardButton("➕ مرحله مقدماتی",callback_data=f"mfeature:stage_add:preliminary:{eid}"),InlineKeyboardButton("🏆 مرحله فینال",callback_data=f"mfeature:stage_add:final:{eid}")); kb.row(InlineKeyboardButton("🤖 گروه‌بندی خودکار",callback_data=f"mfeature:auto_group:{eid}"),InlineKeyboardButton("✋ گروه‌بندی دستی",callback_data=f"mfeature:manual_group:{eid}")); kb.row(InlineKeyboardButton("⭐ امتیازات",callback_data=f"mfeature:event_scores:{eid}"),InlineKeyboardButton("🔄 جایگزینی",callback_data=f"mfeature:event_replace:{eid}")); kb.row(InlineKeyboardButton("❌ لغو اونت",callback_data=f"mfeature:event_cancel:{eid}"),InlineKeyboardButton("🏁 اتمام اونت",callback_data=f"mfeature:event_finish:{eid}")); kb.add(InlineKeyboardButton("⬅️ اونت",callback_data=f"mfeature:event:{eid}")); await callback.message.edit_text(f"⚙️ <b>مدیریت اونت</b>\n\n{html.escape(str(e['name']))}",parse_mode="HTML",reply_markup=kb); await callback.answer()
    ...
    def install(self):
        ...
        self._front(self.achievements,lambda c:str(c.data or "")=="profile:advanced:achievements"); self._front(self.profile_override,lambda c:str(c.data or "")=="ustats:profile"); self._front(self.stats_override,lambda c:str(c.data or "")=="ustats:stats")
        self._front(self.tags,lambda c:str(c.data or "")=="mfeature:tags"); self._front(self.tag_toggle,lambda c:str(c.data or "").startswith("mfeature:tag:"))
        ...
        self._front(self.event_scores,lambda c:str(c.data or "").startswith("mfeature:event_scores:")); self._front(self.score_stage,lambda c:str(c.data or "").startswith("mfeature:score_stage:")); self._front(self.score_player,lambda c:str(c.data or "").startswith("mfeature:score_player:")); self._front(self.auto_group,lambda c:str(c.data or "").startswith("mfeature:auto_group:")); self._front(self.stage_add,lambda c:str(c.data or "").startswith("mfeature:stage_add:")); self._front(self.manual_group,lambda c:str(c.data or "").startswith("mfeature:manual_group:")); self._front(self.event_cancel,lambda c:str(c.data or "").startswith("mfeature:event_cancel:")); self._front(self.event_finish,lambda c:str(c.data or "").startswith("mfeature:event_finish:")); self._front(self.event_edit_name,lambda c:str(c.data or "").startswith("mfeature:event_edit_name:")); self._front(self.event_edit_time,lambda c:str(c.data or "").startswith("mfeature:event_edit_time:")); self._front(self.event_replace,lambda c:str(c.data or "").startswith("mfeature:event_replace:"))
        ...
        self.app._mafia_progress_events_installed=True; return True
