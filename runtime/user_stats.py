"""Private profile, public ranking and group/player statistics."""
...
def _profile_text(row, rank):
    ...
    return (
        ...
        f"⭐ امتیاز: {int(row.get('score',0)) if row else 0}\n"
...
class UserStats:
    ...
    async def show_stats(self, message: types.Message, uid: int | None = None, group_id: int | None = None, edit=False):
        ...
        if not row and int(stats.get("games", 0)) == 0:
            ...
            games = int(stats.get("games", 0)); wins = int(stats.get("wins", 0)); losses = int(stats.get("losses", 0)); draws = int(stats.get("draws", 0)); score = int(stats.get("score", 0))
