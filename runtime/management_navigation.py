"""Management navigation and compatibility helpers.

This module keeps the canonical management panel intact while supplying the
few flows that need stateful UI beyond the generic management handlers:
replacement, birthday/restore, and player-facing attendance.
"""
...
def _active_players(management: GameManagement, game: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        r for r in _rows(management, game)
...
async def _render_attendance(management: GameManagement, game: dict[str, Any]) -> bool:
    ...
    for row in sorted(players, key=lambda r: int(r.get("seat") or 999)):
        ...
        lines.append(f"{icon} {int(row['seat']):02d}. {_mention(row)}")
    if not players:
...
async def _notify_all_ready(management: GameManagement, game: dict[str, Any]) -> None:
    ...
    text = "🎉 <b>همه بازیکنان آماده‌اند.</b>"
    if moderator_id:
        text += f"\n🎩 گرداننده: {_mention({'player_id': moderator_id})}"
    try:
        await management.app.bot.send_message(gid, text, parse_mode="HTML")
    except Exception:
        logging.exception("all-ready group notification failed game=%s", game.get("id"))
    if moderator_id:
        try:
            await management.app.bot.send_message(
                moderator_id,
                f"🎉 بازی شماره {int(game.get('event_number') or 1)}: همه بازیکنان آماده‌اند.",
                parse_mode="HTML",
            )
        except Exception:
            pass
...
def install(app: Any, management: GameManagement) -> bool:
    """Install the missing management flows before GameManagement.install()."""

    async def close_to_lobby(self: GameManagement, callback: Any):
    ...
    async def replace(self: GameManagement, callback: Any):
        ...
        if not game or not await self._allowed(callback, gid, game):
            ...
            return
        substitutes = [
            r for r in _rows(self, game)
            if r.get("seat") is None
            and str(r.get("status") or "") in {"waiting", "substitute"}
            and bool(r.get("is_substitute", False))
        ]
        kb = InlineKeyboardMarkup(row_width=2)
        for row in substitutes:
            kb.insert(InlineKeyboardButton(
                _name(self, row),
                callback_data=f"mgmt:{int(game['id'])}:replace_sub:{int(row['player_id'])}",
            ))
    ...
    async def replace_sub(self: GameManagement, callback: Any):
        ...
        if len(parts) != 4 or parts[0] != "mgmt" or parts[2] != "replace_sub":
            return
        gid = int(callback.message.chat.id)
        game = self._game(gid)
        substitute_id = int(parts[3])
        if not game or not await self._allowed(callback, gid, game):
            await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
            return
        rows = _rows(self, game)
        substitute = next((r for r in rows if int(r["player_id"]) == substitute_id), None)
        if not substitute or substitute.get("seat") is not None or not bool(substitute.get("is_substitute", False)):
            await callback.answer("❌ بازیکن جایگزین معتبر نیست.", show_alert=True)
            return
        targets = [
            r for r in rows
            if r.get("seat") is not None
            and str(r.get("status") or "") not in {"removed", "dead", "finished"}
        ]
        kb = InlineKeyboardMarkup(row_width=2)
        for row in sorted(targets, key=lambda r: int(r.get("seat") or 999)):
            kb.insert(InlineKeyboardButton(
                f"{int(row['seat'])}. {_name(self, row)}",
                callback_data=f"mgmt:{int(game['id'])}:replace_target:{substitute_id}:{int(row['player_id'])}",
            ))
        kb.row(InlineKeyboardButton("⬅️ مدیریت", callback_data=f"mgmt:{int(game['id'])}:open"))
        await callback.message.edit_text(
            f"🔄 <b>انتخاب بازیکن هدف</b>\n\nجایگزین: {_mention(substitute)}\n\nبازیکنی که باید جایگزین شود را انتخاب کنید:",
    ...
    async def replace_target(self: GameManagement, callback: Any):
        parts = str(callback.data or "").split(":")
        if len(parts) != 5 or parts[0] != "mgmt" or parts[2] != "replace_target":
            return
        gid = int(callback.message.chat.id)
        game = self._game(gid)
        substitute_id, target_id = int(parts[3]), int(parts[4])
        if not game or not await self._allowed(callback, gid, game):
            await callback.answer("⛔ دسترسی ندارید.", show_alert=True)
            return
        rows = _rows(self, game)
        ...
        if not substitute or not target or substitute.get("seat") is not None or target.get("seat") is None:
            await callback.answer("❌ اطلاعات جایگزینی نامعتبر است.", show_alert=True)
            return
        ...
        try:
            ...
            attendance = _attendance_state(self, game)
            attendance.pop(str(target_id), None)
            attendance[str(substitute_id)] = False
            self._save(game, attendance=attendance)
        except Exception:
            logging.exception("management replacement failed game=%s", game.get("id"))
            await callback.answer("❌ جایگزینی انجام نشد.", show_alert=True)
            return
        await callback.message.edit_text(
            f"✅ {_mention(target)} با {_mention(substitute)} جایگزین شد.\n💺 صندلی {seat}",
            parse_mode="HTML", reply_markup=self.panel(game["id"]),
        )
        await _refresh_lobby_if_needed(self, game)
    ...
    async def birthday(self: GameManagement, callback: Any):
        ...
        rows = [
            r for r in _rows(self, game)
            if str(r.get("status") or "") in {"dead", "removed"} or not bool(r.get("is_alive", True))
        ]
        kb = InlineKeyboardMarkup(row_width=2)
        for row in rows:
    ...
    async def birthday_pick(self: GameManagement, callback: Any):
        parts = str(callback.data or "").split(":")
        if len(parts) != 4 or parts[0] != "mgmt" or parts[2] != "birthday_pick":
            return
        gid = int(callback.message.chat.id)
        game = self._game(gid)
        uid = int(parts[3])
        ...
        if seat is None:
            ...
            scenario = self.scenarios.get_by_id(int(scenario_id)) if scenario_id else None
            capacity = len((scenario or {}).get("roles") or [])
            occupied = {
                int(r["seat"]) for r in rows
                if r.get("seat") is not None
                and int(r["player_id"]) != uid
                and str(r.get("status") or "") not in {"removed", "dead", "finished"}
            }
            seat = next((n for n in range(1, capacity + 1) if n not in occupied), None)
        if seat is None:
            await callback.answer("❌ صندلی خالی برای بازگشت این بازیکن وجود ندارد.", show_alert=True)
            return
        try:
            self.app.runtime.state.games.set_player_seat(game["id"], uid, int(seat))
            self.app.runtime.state.games.set_player_status(game["id"], uid, "active")
            self.app.runtime.state.games.set_player_alive(game["id"], uid, True)
            attendance = _attendance_state(self, game)
            attendance[str(uid)] = False
            self._save(game, attendance=attendance)
        except Exception:
            logging.exception("birthday restore failed game=%s user=%s", game.get("id"), uid)
            await callback.answer("❌ بازگردانی بازیکن انجام نشد.", show_alert=True)
            return
        await callback.message.edit_text(
            f"🎂 {_mention(player)} دوباره وارد بازی شد.\n💺 صندلی {int(seat)}",
            parse_mode="HTML", reply_markup=self.panel(game["id"]),
        )
        await _refresh_lobby_if_needed(self, game)
        await _render_attendance(self, game)
        await callback.answer("🎂 بازیکن به بازی برگشت.")
    ...
    async def attendance(self: GameManagement, callback: Any):
        ...
        lines = ["🟢 <b>بازیکنان حاضر در لیست</b>", ""]
        for row in sorted(players, key=lambda r: int(r.get("seat") or 999)):
            lines.append(f"⚪️ {int(row['seat']):02d}. {_mention(row)}")
        lines.extend(["", "لطفاً برای اعلام حاضری، «آماده‌ام» را بزنید."])
        kb = InlineKeyboardMarkup(row_width=1)
        kb.add(InlineKeyboardButton("آماده‌ام ✅", callback_data=f"mgmt:{int(game['id'])}:attendance_ready"))
        ...
        try:
            if old_id:
                await self.app.bot.edit_message_text("\n".join(lines), gid, int(old_id), parse_mode="HTML", reply_markup=kb)
                message_id = int(old_id)
            else:
                msg = await self.app.bot.send_message(gid, "\n".join(lines), parse_mode="HTML", reply_markup=kb)
                message_id = int(msg.message_id)
            self._save(game, attendance_message_id=message_id)
            await callback.answer("✅ لیست حاضری ایجاد شد.")
        except Exception:
            logging.exception("attendance message creation failed game=%s", game.get("id"))
            await callback.answer("❌ ایجاد لیست حاضری انجام نشد.", show_alert=True)
    ...
    async def attendance_ready(self: GameManagement, callback: Any):
        ...
        await _render_attendance(self, game)
        players = _active_players(self, game)
        all_ready = bool(players) and all(bool(attendance.get(str(int(r["player_id"])), False)) for r in players)
        if all_ready:
            await _notify_all_ready(self, game)
        await callback.answer("✅ آمادگی شما ثبت شد.")

    GameManagement.close = close_to_lobby
    ...
    # Registered before management.install(), so the generic management
    # dispatcher cannot swallow this player-facing action.
