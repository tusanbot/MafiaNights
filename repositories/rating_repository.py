...
class RatingRepository(DatabaseRepository):
    ...
    def player_profile(self,user_id):
        with self.SessionLocal() as session:
            row=session.execute(text("""
                ...
                select p.user_id,p.username,p.first_name,p.last_name,p.nickname,coalesce(r.games,0)::int games,
                       (50+coalesce(r.score,0)+(select coalesce(sum(ar.reward_points),0) from public.mafia_achievement_rewards ar where ar.player_id=p.user_id))::int score,
                       (select coalesce(sum(ar.reward_points),0) from public.mafia_achievement_rewards ar where ar.player_id=p.user_id)::int achievement_points,
                       coalesce(r.wins,0)::int wins,coalesce(r.losses,0)::int losses,coalesce(r.draws,0)::int draws
                from public.mafia_players p left join r on r.user_id=p.user_id where p.user_id=:user_id limit 1
    ...
    def rank(self,user_id):
        with self.SessionLocal() as session:
            row=session.execute(text("""
                with totals as (select p.user_id,(50+coalesce(sum(r.score),0)+(select coalesce(sum(ar.reward_points),0) from public.mafia_achievement_rewards ar where ar.player_id=p.user_id))::int score,count(r.id)::int games,count(r.id) filter(where r.result='win')::int wins from public.mafia_players p left join public.mafia_ratings r on r.user_id=p.user_id group by p.user_id)
    ...
    def top(self,limit=10):
        with self.SessionLocal() as session:
            rows=session.execute(text("""
                select p.user_id,coalesce(p.nickname,p.first_name,p.username,p.user_id::text) name,count(r.id)::int games,
                       ...
                       (select coalesce(sum(ar.reward_points),0) from public.mafia_achievement_rewards ar where ar.player_id=p.user_id)::int achievement_points,count(r.id) filter(where r.result='win')::int wins
                from public.mafia_players p left join public.mafia_ratings r on r.user_id=p.user_id group by p.user_id,p.nickname,p.first_name,p.username order by score desc,wins desc,games desc,p.user_id limit :limit
    ...
    def group_top(self,group_chat_id,limit=10):
        with self.SessionLocal() as session:
            rows=session.execute(text("""
                select p.user_id,coalesce(p.nickname,p.first_name,p.username,p.user_id::text) name,count(r.id)::int games,
                       (50+coalesce(sum(r.score),0)+(select coalesce(sum(ar.reward_points),0) from public.mafia_achievement_rewards ar where ar.player_id=p.user_id))::int score,count(r.id) filter(where r.result='win')::int wins
                from public.mafia_players p join public.mafia_ratings r on r.user_id=p.user_id join public.mafia_games g on g.id=r.game_id where g.group_chat_id=:group_chat_id
                group by p.user_id,p.nickname,p.first_name,p.username order by score desc,wins desc,games desc,p.user_id limit :limit
