...
class PlayerRepository(DatabaseRepository):
        ...
        with self.SessionLocal() as session:
            session.execute(text("""
                insert into public.mafia_players
                    (id, user_id, username, first_name, last_name, updated_at)
                values
                    (:id, :user_id, :username, :first_name, :last_name, now())
                on conflict (id) do update set
                    ...
                    updated_at = now()
            """), {"id": user_id, "user_id": user_id, "username": username, "first_name": first_name, "last_name": last_name})
            session.commit()
    ...
    def register(self, user_id, nickname, username=None, first_name=None, last_name=None):
        ...
        with self.SessionLocal() as session:
            session.execute(text("""
                insert into public.mafia_players
                    (id, user_id, username, first_name, last_name, nickname, updated_at)
                values
                    (:id, :user_id, :username, :first_name, :last_name, :nickname, now())
                on conflict (id) do update set
                    username = coalesce(excluded.username, public.mafia_players.username),
                    first_name = coalesce(excluded.first_name, public.mafia_players.first_name),
                    last_name = coalesce(excluded.last_name, public.mafia_players.last_name),
    ...
    def get(self, user_id):
        with self.SessionLocal() as session:
            row = session.execute(text("""
                select id, username, first_name, last_name, nickname, gender
    ...
    def get_display_name(self, user_id, fallback="❓"):
        ...
        if nickname:
            return nickname
        real_name = " ".join(p for p in ((row.get("first_name") or "").strip(), (row.get("last_name") or "").strip()) if p)
    ...
    def set_nickname(self, user_id, nickname):
        ...
        if not nickname:
            return False
        with self.SessionLocal() as session:
            result = session.execute(text("update public.mafia_players set nickname=:nickname, updated_at=now() where id=:id"), {"id": int(user_id), "nickname": nickname})
            session.commit()
            return result.rowcount > 0

    def delete_nickname(self, user_id):
        with self.SessionLocal() as session:
            result = session.execute(text("update public.mafia_players set nickname=null, updated_at=now() where id=:id"), {"id": int(user_id)})
            session.commit()
            return result.rowcount > 0
    ...
    def all_nicknames(self):
        with self.SessionLocal() as session:
            rows = session.execute(text("""
                ...
                where nickname is not null and trim(nickname) <> '' order by lower(nickname)
            """)).mappings().all()
